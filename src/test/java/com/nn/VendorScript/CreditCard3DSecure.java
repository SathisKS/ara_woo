 ...
 * Description:  This file is used to execute vendor script for Credit Card 3D Secure
 * Author:       Novalnet AG
...
public class CreditCard3DSecure extends TestInputData {
	TestInputData Data = new TestInputData();

	// Method used to execute 'CREDITCARD_BOOKBACK'
	@Test
	public void creditCard3DBookback() throws Exception {
		try {
			...
			test = extend.createTest("Vendor script execution for Credit Card 3D Secure 'CREDITCARD_BOOKBACK'");
			// Steps
			...
			// Enable 3D Secure For credit card Payment
			Actions action1 = new Actions(driver);
			Thread.sleep(3000);
			WebElement cc3D = element.Creditcard_Enable_3d_Secure_Selectbox;
			Thread.sleep(5000);
			action1.click(cc3D).sendKeys("Yes", Keys.DOWN, Keys.ENTER).build().perform();
			Thread.sleep(2000);
			// Enable Capture
			...
			if (element.Creditcard_Title.isDisplayed() == true) {
				...
				// Switch to cc3d Redirect iframe
				driver.switchTo().frame("authWindow");
				element.CC3D_Password_TextBox.sendKeys("12345");
				element.CC3D_Submit_Button.click();
				// Switch to default window
				driver.switchTo().defaultContent();
				...
				if (thankyoumessage.equals("Thank you. Your order has been received.")) {
					System.out.println("Order placed successfully using Credit Card 3D Secure");
					test.log(Status.INFO, "Order placed successfully using Credit Card 3D Secure");
					Thread.sleep(4000);
					...
					System.out.println("TC FAILED: Order was not placed successfully using Credit Card 3D Secure");
					test.log(Status.FAIL, "TC FAILED: Order was not placed successfully using Credit Card 3D Secure");
				}
			}
			// Close browser
			Utility.closeBrowser();
		} catch (Exception e) {
			System.out.println("ERROR: Unexpected error from 'creditCard3DBookback' method");
			test.log(Status.ERROR, "ERROR: Unexpected error from 'creditCard3DBookback");
		}
	}

	// Method used to execute 'CREDITCARD_CHARGEBACK'
	@Test
	public void creditCard3DChargeBack() throws Exception {
		try {
			// Launch the browser and load the default URL
			Utility.initConfiguration();
			Thread.sleep(3000);
			...
			ElementLocators element = PageFactory.initElements(driver, ElementLocators.class);
			// Title for HTML report
			test = extend.createTest("Vendor script execution for Credit Card 3D Secure 'CREDITCARD_CHARGEBACK'");
			// Steps
			Actions actions = new Actions(driver);
			actions.moveToElement(element.WooCommerce).perform();
			Thread.sleep(3000);
			...
			action1.click(cc3D).sendKeys("Yes", Keys.DOWN, Keys.ENTER).build().perform();
			Thread.sleep(2000);
			// If On-hold enabled then it will disable
			Actions action = new Actions(driver);
			Thread.sleep(3000);
			WebElement onhold = element.Creditcard_Pending_Payment_Status_Selectbox;
			Thread.sleep(5000);
			...
			System.out.println("ERROR: Unexpected error from 'creditCard3DChargeBack' method");
			test.log(Status.ERROR, "ERROR: Unexpected error from 'creditCard3DChargeBack' method");
		}
	}

	// Method used to execute 'CREDIT_ENTRY_CREDITCARD'
	@Test
	public void creditEntryCreditCard3D() throws Exception {
		try {
			// Launch the browser and load the default URL
			Utility.initConfiguration();
			Thread.sleep(3000);
			...
			ElementLocators element = PageFactory.initElements(driver, ElementLocators.class);
			// Title for HTML report
			test = extend.createTest("Vendor script execution for Credit Card 3D Secure 'CREDIT_ENTRY_CREDITCARD'");
			// Steps
			Actions actions = new Actions(driver);
			actions.moveToElement(element.WooCommerce).perform();
			Thread.sleep(3000);
			...
			System.out.println("ERROR: Unexpected error from 'creditEntryCreditCard3D' method");
			test.log(Status.ERROR, "ERROR: Unexpected error from 'creditEntryCreditCard3D' method");
		}
	}

	// Method used to execute 'CREDITCARD' onhold to confirm
	@Test
	public void creditcard3DOnholdToComfirm() throws Exception {
		try {
			// Launch the browser and load the default URL
			Utility.initConfiguration();
			Thread.sleep(3000);
			...
			// Title for HTML report
			test = extend
					.createTest("Vendor script execution for Credit Card 3D Secure 'CREDITCARD' onhold to confirm");
			// Steps
			Actions actions = new Actions(driver);
			actions.moveToElement(element.WooCommerce).perform();
			Thread.sleep(3000);
			...
			System.out.println("ERROR: Unexpected error from 'creditcard3DOnholdToComfirm' method");
			test.log(Status.ERROR, "ERROR: Unexpected error from 'creditcard3DOnholdToComfirm' method");
		}
	}
}